...
/// Debug flag to log all NgSpice commands
const DEBUG_NGSPICE_COMMANDS: bool = false;
...
impl NgSpice {
    ...
    pub fn command(&self, cmd: &str) -> Result<(), String> {
        if DEBUG_NGSPICE_COMMANDS {
            // Show escaped version to see newlines
            let escaped = cmd.replace('\n', "\\n");
            println!("[NGSPICE CMD] {}", escaped);
            println!("[NGSPICE CMD LINES] {}", cmd.lines().count());
        }
        unsafe {
